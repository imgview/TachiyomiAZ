...
class LocalFavoritesStorage {
    ...
    private fun getChangedEntries(
        realm: Realm,
        entries: Sequence<FavoriteEntry>
    ): ChangeSet {
        ...
        val removed =
            realm.where(FavoriteEntry::class.java)
                .findAll()
                .filter {
    ...
    private fun queryListForEntry(
        list: List<FavoriteEntry>,
        entry: FavoriteEntry
    ) = list.find {
        it.gid == entry.gid &&
    ...
    private fun validateDbManga(manga: Manga) = manga.favorite && (manga.source == EH_SOURCE_ID || manga.source == EXH_SOURCE_ID)
