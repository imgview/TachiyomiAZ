...
inline fun <reified E : RealmModel> RealmQuery<out E>.beginLog(
    clazz: Class<out E>? =
        E::class.java
): LoggingRealmQuery<out E> = LoggingRealmQuery.fromQuery(this, clazz)

class LoggingRealmQuery<E : RealmModel>(val query: RealmQuery<E>) {
    companion object {
        fun <E : RealmModel> fromQuery(
            q: RealmQuery<out E>,
            clazz: Class<out E>?
    ...
    private fun appendGreaterThanOrEqualTo(
        fieldName: String,
        value: Any?
    ) {
        log += sec("\"$fieldName\" >= $value")
    ...
    private fun appendLessThanOrEqualTo(
        fieldName: String,
        value: Any?
    ) {
        log += sec("\"$fieldName\" <= $value")
